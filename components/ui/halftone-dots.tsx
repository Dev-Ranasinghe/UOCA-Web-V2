...
// ShaderBackground — generated by the 21st.dev Shader Builder ("Halftone Dots").
// Zero dependencies: one WebGL canvas that fills its parent. Drop it behind your content with
// <div className="relative"><HalftoneDots className="absolute inset-0" />…
// Adapted from Paper Shaders: https://shaders.paper.design/halftone-dots
// Paper Shaders is licensed under Apache-2.0.
//
// Changes from the generated version: the palette is a prop (the default is the original grey ramp), the dot
// pitch can be fixed in CSS pixels (`dotSize`), reduced-motion visitors get a still frame, and shader compile/link
...
/** The original grey ramp: first colour is the background, the rest are the dots' shades, dark to light. */
export const HALFTONE_GREYS = ["#111111", "#525252", "#d4d4d4", "#fafafa"];
...
/** Time (seconds) of the still frame shown to visitors who prefer reduced motion. */
...
export interface HalftoneDotsProps {
  className?: string;
  /**
   * Up to 8 hex colours. The first is the background; the rest are the dot shades from dark to light.
   * Defaults to the generated grey ramp.
   */
  colors?: string[];
  /**
   * Distance between dot centres, in CSS pixels. Left unset, the grid is sized to the canvas (about 35 rows across its
   * shorter side), so a tall section gets big dots. Set it to keep the same fine grid at any size.
   */
  dotSize?: number;
}

export function HalftoneDots({ className, colors = HALFTONE_GREYS, dotSize }: HalftoneDotsProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Keyed by value so a fresh array each render doesn't rebuild the GL program.
  ...
  useEffect(() => {
    ...
    if (!gl) return;
    // Non-null aliases for the hoisted `render` function, where TypeScript drops the narrowing above.
    ...
    const compile = (type: number, src: string) => {
      ...
      if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        console.error("HalftoneDots shader failed to compile:", gl.getShaderInfoLog(s));
      }
      return s;
    };
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error("HalftoneDots program failed to link:", gl.getProgramInfoLog(program));
    ...
    // Exactly eight vec3s: extra colours are dropped, missing ones repeat the last.
    const list = colorsKey ? colorsKey.split(",").slice(0, 8) : HALFTONE_GREYS;
    const colorCount = Math.max(1, list.length);
    while (list.length < 8) list.push(list[list.length - 1]);
    gl.uniform3fv(uni.colors, new Float32Array(list.flatMap(hexToVec3)));
    ...
    // A screen early, so the GPU's first-draw warm-up happens off screen rather than as a hitch on scroll-in.
    ...
    function render(now: number) {
      ...
      const seconds = reduceMotion ? STILL_FRAME_SECONDS : (now - start) / 1000;
      ctx.uniform4f(uni.scene, width, height, seconds * UNIFORMS.timeScale, colorCount);
      // Fixed dot pitch: cells per shorter side = 13 + intensity * 42, so scale = shorter side / (pitch * cells).
      const cells = 13 + UNIFORMS.intensity * 42;
      const scale =
        dotSize && dotSize > 0
          ? Math.max(0.2, Math.min(bounds.width, bounds.height) / (dotSize * cells))
          : UNIFORMS.scale;
      ctx.uniform4f(uni.shape, scale, UNIFORMS.intensity, UNIFORMS.paramA, UNIFORMS.warp);
    ...
    return () => {
      ...
      gl.deleteBuffer(buf);
      gl.deleteProgram(program);
      // Non-null aliases: TypeScript drops the narrowing inside the timer callback.
      const releaseCanvas: HTMLCanvasElement = canvas;
      const releaseGl: WebGLRenderingContext = gl;
      const releaseTimer = window.setTimeout(() => {
        if (pendingContextReleases.get(releaseCanvas) !== releaseTimer) return;
        pendingContextReleases.delete(releaseCanvas);
        releaseGl.getExtension("WEBGL_lose_context")?.loseContext();
        releaseCanvas.width = 1;
        releaseCanvas.height = 1;
      }, 0);
      pendingContextReleases.set(releaseCanvas, releaseTimer);
    };
  }, [colorsKey, dotSize]);
...
// The name used by the generated component.
export { HalftoneDots as ShaderBackground };
