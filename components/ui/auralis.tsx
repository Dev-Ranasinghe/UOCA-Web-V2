...
export interface AuralisProps {
  /** Up to three hex colours: [0] broad glow, [1] bright streaks, [2] reserved. */
  colors?: string[];
  speed?: number;
  grain?: number;
  height?: string;
  className?: string;
  /** Rendered above the animated background. */
  children?: ReactNode;
}
...
/** Exactly three vec3s: extra colours are dropped, missing ones repeat the last. */
function flattenColors(colors: string[]): Float32Array {
  const list = colors.length ? colors.slice(0, 3) : DEFAULT_COLORS;
  while (list.length < 3) list.push(list[list.length - 1]);
  return new Float32Array(list.flatMap(hexToRgb));
}
...
const Auralis = ({
  ...
  // Keyed by value so passing a fresh array each render doesn't rebuild the GL program.
  ...
  useEffect(() => {
    ...
    // These never change between frames.
    gl.uniform1f(locs.grain, grain);
    gl.uniform3fv(locs.colors, flattenColors(colorsKey.split(",")));

    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    let raf = 0;
    let visible = true;
    // A still frame for reduced motion; otherwise the clock runs from mount.
    const STILL_FRAME_MS = 4000;

    const draw = (ms: number) => {
      gl.uniform2f(locs.res, canvas.width, canvas.height);
      gl.uniform1f(locs.time, ms * 0.001 * speed);
    ...
    const resize = () => {
      ...
      // A resize clears the canvas; repaint at once so a paused or still background never blanks.
      draw(reduceMotion ? STILL_FRAME_MS : performance.now());
    };
    ...
    return () => {
      ...
      io.disconnect();
      stop();
      // Release only what this effect created. The context itself is left alone: React Strict Mode
      // re-runs the effect on the same canvas, and a context we had force-lost can't be reused.
      gl.deleteBuffer(buffer);
      gl.deleteProgram(program);
      gl.deleteShader(vertex);
      gl.deleteShader(fragment);
    };
  }, [colorsKey, speed, grain]);
