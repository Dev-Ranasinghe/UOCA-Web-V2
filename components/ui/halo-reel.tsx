...
function WheelCard({
  ...
  // Equal angle steps bunch cards near the front (cos flattens out there) and
  // spread them apart near the sides (cos steepens there). Easing the value
  // that drives X — but not Y or scale — evens out the side-to-side gaps
  // while leaving the vertical curve and depth falloff untouched.
  const x = useTransform(cos, (c) => Math.sign(c) * Math.pow(Math.abs(c), 1.3) * radiusX);
  ...
  return (
    ...
    >
      {item.src ? (
        <div className="flex h-full w-full flex-col border border-[#121212] bg-[#f7f5f0] p-[7%]">
          {/* Postcard-style "ooo ---" stamp header, matching PostCard/team cards */}
          <div className="mb-[6%] flex items-center gap-1 font-mono text-[7px] font-semibold leading-none tracking-[1px] text-[#121212]">
            <span>ooo</span>
            <span className="h-0 flex-1 border-b border-dashed border-[#121212]" />
          </div>
          <div className="relative flex-1 overflow-hidden border border-[#121212] bg-[#e0ddd5]">
            <img
              src={item.src}
        ...
        <div
          className="flex h-full w-full flex-col border border-[#121212] bg-card p-[7%] text-card-foreground"
          style={{
            backgroundColor: item.bgColor,
            color: item.textColor,
          }}
        >
          <div className="mb-[6%] flex items-center gap-1 font-mono text-[7px] font-semibold leading-none tracking-[1px] opacity-80">
            <span>ooo</span>
            <span className="h-0 flex-1 border-b border-dashed border-current" />
          </div>
          <div className="flex flex-1 flex-col items-center justify-center gap-1 text-center">
            ...
            {item.subtitle ? (
              <span className="font-mono text-[0.6rem] uppercase tracking-[0.2em] opacity-70">
                {item.subtitle}
              </span>
            ) : null}
          </div>
